...
function createTimeSlot(startTime, endTime, isFirstRow = false, lifeTag = null) {
    ...
    function populateDropdown() {
        ...
        // 根据时间段过滤标签
        // 非生活时间段（8-12点，14-19点，21-22点）默认不显示生活类标签（color为null），
        // 但用户要求全时段可用的默认生活标签（always:true，如午餐/晚餐/通勤/锻炼/冥想/洗漱）始终显示
        const filteredTags = isLifeTimeSlot(startHour) ? tags : tags.filter(tag => tag.color !== null || tag.always);
...
function updateTimeStatistics() {
    ...
    // 生活任务关键词列表
    const lifeKeywords = ['做饭', '吃饭', '洗澡', '打扫卫生', '打扫', '卫生', '出门购物', '购物', '买菜', '洗衣', '洗碗', '拖地', '擦桌子', '整理房间', '倒垃圾', '遛狗', '喂猫', '照顾孩子', '接孩子', '送孩子', '陪孩子', '做家务', '家务', '生活', '休息', '睡觉', '晨跑', '洗漱', '午休', '午睡', '冥想', '瑜伽', '健身', '锻炼', '运动', '跑步', '散步', '通勤', '上班路上', '下班路上', '早餐', '午餐', '晚餐', '做饭', '备餐', '收拾', '整理', '清洁', '护肤', '化妆', '更衣', '穿衣', '午休', '小憩'];
    ...
    scheduleRows.forEach(row => {
        ...
        // 解析时间段，计算该时间段的总分钟数（现在为30分钟）
        const timeSlotMinutes = 30; // 每个时间段都是30分钟
...
function formatTimeStatsForGoalCompletion(timeStats) {
    ...
    // 生活任务关键词列表
    const lifeKeywords = ['做饭', '吃饭', '洗澡', '打扫卫生', '打扫', '卫生', '出门购物', '购物', '买菜', '洗衣', '洗碗', '拖地', '擦桌子', '整理房间', '倒垃圾', '遛狗', '喂猫', '照顾孩子', '接孩子', '送孩子', '陪孩子', '做家务', '家务', '生活', '休息', '睡觉', '晨跑', '洗漱', '午休', '午睡', '冥想', '瑜伽', '健身', '锻炼', '运动', '跑步', '散步', '通勤', '上班路上', '下班路上', '早餐', '午餐', '晚餐', '做饭', '备餐', '收拾', '整理', '清洁', '护肤', '化妆', '更衣', '穿衣', '小憩'];
...
function updatePieChart(typeDurations, totalMinutes) {
    ...
    const lifeKeywords = ['做饭', '吃饭', '洗澡', '打扫卫生', '打扫', '卫生', '出门购物', '购物', '买菜', '洗衣', '洗碗', '拖地', '擦桌子', '整理房间', '倒垃圾', '遛狗', '喂猫', '照顾孩子', '接孩子', '送孩子', '陪孩子', '做家务', '家务', '生活', '休息', '睡觉', '晨跑', '洗漱', '午休', '午睡', '冥想', '瑜伽', '健身', '锻炼', '运动', '跑步', '散步', '通勤', '上班路上', '下班路上', '早餐', '午餐', '晚餐', '备餐', '收拾', '整理', '清洁', '护肤', '化妆', '更衣', '穿衣', '小憩'];
...
const TAG_MANAGER_CONFIG = {
    ...
    defaultTags: [
        ...
        { id: 'tag_10', name: '锻炼', color: null, always: true },
    ...
    try {
        ...
        if (storedTags) {
            ...
            if (!localStorage.getItem(TAG_MANAGER_CONFIG.defaultsKey)) {
                ...
                const toAdd = TAG_MANAGER_CONFIG.defaultTags.filter(t => !existingIds.has(t.id) && !existingNames.has(t.name));
                if (toAdd.length > 0) {
                    tags = tags.concat(toAdd);
                    saveUserTags(tags);
                    console.log('【标签管理】已一次性追加新版默认生活标签:', toAdd.map(t => t.name).join('、'));
                }
            }
            // 过滤掉需要删除的标签：学习和休息放松
